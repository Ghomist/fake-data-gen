...
- [faker](https://github.com/joke2k/faker)：快速生成各种样式的假数据，并且支持 i18n
- [xeger](https://github.com/crdoconnor/xeger)：反向 regex，通过正则表达式生成随机字符串
...
使用这个脚本需要了解 yaml 的基本格式，以及少量的 python 语法，某些地方可以直接写 python 表达式来生成数据

## 快速开始

克隆源码目录，运行 `main.py`，不出意外会在当前目录下读取 [`sample_schema.yaml`](./sample_schema.yaml) 和 [`sample_provider.txt`](./sample_provider.txt)，他们是项目内置的示例文件

```bash
git clone https://github.com/Ghomist/fake-data-gen.git
cd fake-data-gen
python main.py
```
...
params:
  # 生成数据的数量（此处不填的话默认为10）
  amount: 30
  # 生成失败时重新尝试的最大次数（一般是数据设有 limit 时会失败，详见后文）
  max_try: 100
  # 输出的格式，目前支持 print、csv、sql，详见下文
  format: csv
...
其基本结构是：struct 下是多个表名，每个表名下是它对应的字段名，然后每个字段下的对象是一个**生成器对象**，如果字段下是一个列表的话，那么视为使用了多个生成器，列表中的每一项都应符合生成器的声明格式
...
目前脚本中的“足够多次”默认为100次，可在 schema 文件的 `params` 部分自行调整
...
其中 line 对象有一定的使用限制，它所能够引用的只有已经生成的列的数据，但有时候靠前的数据列也需要依据靠后的列进行条件判断等，此时可以在生成器声明中加上 order 选项，具体用法见下文

## 字段生成顺序

由于语境 context 中可以通过 line 等对象引用到其它字段，但是由于脚本仅按列生成，位置靠前的列会优先被生成，而位于后面的列还没有生成时，显然无法进行引用，此时可以使用 order 选项，改变这个字段的生成顺序
...
有时我们规定的生成规则可能过于广泛，或者需要额外的规则进行约束（例如生成身份证号时，可能需要保证其格式正确），此时可以使用 limit 选项

生成器在生成结果后，会把结果赋值给名为 value 的变量（注意此变量不存在于全局语境中，仅在 limit 内有效），并且随全局语境一起代入 limit 的表达式当中进行验算，如果能够满足条件（表达式结果为真），那么该字段就生成完毕，如果不满足条件，生成器会尝试重新生成，直到超过最大尝试次数

```yaml
# 某月中的某一天，可能的范围在1到31之间
day_of_month:
  type: int  # 强制转换至整数
  rule: regex
  value: \d{2}  # 生成2位数，例如 27 表示某月27日
  limit: int(value) > 0 and int(value) <= 31

# 上面这个例子可以有更简单的写法，其只是为了说明 limit 选项的用法
# 上面的例子完全等同于下面的这个
day_of_month:
  rule: range
  value: 1...31
...
更好的例子可以查看 [`sample_schema.yaml`](./sample_schema.yaml)
